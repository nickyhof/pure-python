...
writes derived-property bodies in a uniform arrow form -- variables (``$x``),
literals, property access (``.name``) and function application
(``->fn(args)``) -- so this only needs to re-parse those shapes.
...
Scope (slice 1): single ``$this``/variable, literals, arithmetic / comparison /
function application in arrow form, and property access. Infix operators are not
parsed because the emitter never produces them.
"""
...
def parse_expression(text: str) -> m3.ValueSpecification:
    ...
    return _lower_expression(lines[0].combinedExpression())


def _lower_expression(combined) -> m3.ValueSpecification:
    expr = combined.expressionOrExpressionGroup().expression()
...
def _lower_atomic(nae) -> m3.ValueSpecification:
    ...
    if atom is None:
        ...
        if combined is not None:
            return _lower_expression(combined)
...
def _lower_property_or_function(receiver, pof) -> m3.ValueSpecification:
    ...
    for name_ctx, params in zip(names, params_list):
        ...
        args = [_lower_expression(c) for c in params.combinedExpression()]
...
def _lower_literal(literal_token) -> object:
    if literal_token.STRING() is not None:
        return literal_token.STRING().getText()[1:-1].replace("\\'", "'")
